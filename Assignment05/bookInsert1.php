<?php
    ...
    function getTypeSelect() {
        global $conn;
        $sql = "SELECT * FROM typebook ORDER BY typeId"; // แก้ไขจาก booktype เป็น typebook
        ...
        while ($result = mysqli_fetch_object($dbQuery)) {
            echo '<option value=' . $result->TypeID . '>' . $result->TypeName . '</option>'; // แก้ไขตัวพิมพ์เล็กพิมพ์ใหญ่
        }
    }

    function getStatusSelect() {
        global $conn;
        $sql = "SELECT * FROM statusbook ORDER BY statusId"; // แก้ไขจาก bookstatus เป็น statusbook
        $dbQuery = mysqli_query($conn, $sql);
        if (!$dbQuery) die("(functionDB:getStatusSelect) select status มีข้อผิดพลาด");
        echo '<option value="">เลือกสถานะ</option>';
        while ($result = mysqli_fetch_object($dbQuery)) {
            echo '<option value=' . $result->StatusID . '>' . $result->StatusName . '</option>'; // แก้ไขตัวพิมพ์เล็กพิมพ์ใหญ่
        }
    }
?>
